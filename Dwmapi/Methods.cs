...
// ReSharper restore CheckNamespace
{
	...
	/// <summary>
	/// Collection of native methods in dwmapi.dll.
	/// </summary>
	public class Dwmapi
	{
		/// <summary>
		...
		/// <remarks>
		///     <para>
		/// When creating custom frames that include the standard caption buttons, WM_NCHITTEST and other non-client hit test messages should first be passed to
		/// the DwmDefWindowProc function. This enables the DWM to provide hit testing for the captions buttons. If DwmDefWindowProc does not handle the
		/// non-client hit test messages, further processing of these messages might be necessary.
		///     </para>
		/// </remarks>
		...
		/// <remarks>
		///     <para>
		/// Enabling blur by setting the fEnable member of the DWM_BLURBEHIND structure to TRUE. This results in subsequent compositions of the window blurring
		/// the content behind it. This function should be called immediately before a BeginPaint call to ensure prompt application of the effect.
		///     </para>
		///     <para>
		/// The alpha values in the window are honored and the rendering atop the blur will use these alpha values. It is the application's responsibility to
		/// ensure that the alpha values of all pixels in the window are correct. Some Windows Graphics Device Interface (GDI) operations do not preserve alpha
		/// values, so care must be taken when presenting child windows because the alpha values they contribute are unpredictable.
		///     </para>
		///     <para>
		/// The region specified within the DWM_BLURBEHIND structure is owned by the caller. It is the caller's responsibility to free the region, and they can
		/// do so as soon as the function call is completed.
		///     </para>
		...
		///     <para>
		/// This function must be called whenever Desktop Window Manager (DWM) composition is toggled. Handle the WM_DWMCOMPOSITIONCHANGED message for composition
		/// change notification.
		///     </para>
		/// </remarks>
		...
		/// <remarks>
		///     <para>
		/// Disabling DWM composition disables it for the entire desktop. DWM composition will be automatically enabled when all processes that have disabled
		/// composition have called DwmEnableComposition to enable it or have been terminated. The WM_DWMCOMPOSITIONCHANGED notification is sent whenever DWM
		...
		/// <summary>
		/// Notifies the Desktop Window Manager (DWM) to opt in to or out of Multimedia Class Schedule Service (MMCSS) scheduling while the calling process is
		/// alive.
		/// </summary>
		...
		/// <remarks>
		///     <para>
		/// The value pointed to by pcrColorization is in an 0xAARRGGBB format. Many Microsoft Win32 APIs, such as COLORREF, use a 0x00BBGGRR format. Be careful
		...
		/// <summary>
		/// Obtains a value that indicates whether Desktop Window Manager (DWM) composition is enabled. Applications can listen for composition state changes by
		/// handling the WM_DWMCOMPOSITIONCHANGED notification.
		/// </summary>
		...
		/// <remarks>
		///     <para>
		/// Registering a DWM thumbnail relationship will not modify desktop composition; for information about thumbnail positioning, see the documentation for
		/// the DwmUpdateThumbnailProperties function.
		///     </para>
		///     <para>
		...
		///     </para>
		///     <para>
		/// The thumbnail registration handle obtained by this function is not globally unique but is unique to the process. Call the DwmUnregisterThumbnail
		/// function to unregister the thumbnail. This must be done within the process that the relationship was registered in.
		///     </para>
		/// </remarks>
		...
		/// <remarks>
		///     <para>
		/// The DWM will attempt to display the presented frame for at least the number of monitor refreshes specified. It might be impossible to display the
		/// frame for the precise number of refreshes due to the current composition rate. If the frame is presented late to the DWM or the DWM is late in
		/// composing, a frame could be displayed for fewer than the number of refreshes requested or even skipped completely.
		///     </para>
		/// </remarks>
		...
		/// </param>
		/// <param name="pvAttribute">
		/// A pointer to the value of the attribute specified in the <paramref name="dwAttribute" /> parameter. Different <see cref="DWMWINDOWATTRIBUTE" /> flags
		/// require different value types.
		/// </param>
		/// <param name="cbAttribute">
		...
		/// <remarks>
		///     <para>
		/// Thumbnail relationships created by the DwmRegisterThumbnail function will not be rendered to the destination window until this function is called.
		...
		/// <remarks>
		///     <para>
		/// An application typically calls the DwmSetIconicThumbnail function after it receives a WM_DWMSENDICONICTHUMBNAIL message for its window. The thumbnail
		/// should not exceed the maximum x-coordinate and y-coordinate that are specified in that message. The thumbnail must also have a 32-bit color depth.
		///     </para>
		///     <para>
		/// The application calls DwmInvalidateIconicBitmaps to indicate to the Desktop Window Manager (DWM) that the iconic thumbnail and live preview bitmaps
		/// are out-of-date and should be refreshed. The DWM then requests new versions from the window when they are needed. However, if the DWM bitmap cache is
		/// full, DWM will not request updated versions.
		///     </para>
		///     <para>
		...
		/// window is not notified, but it might receive a subsequent WM_DWMSENDICONICTHUMBNAIL request when its thumbnail is needed again. The caller retains
		/// ownership of the original bitmap and is responsible for freeing the resources that it uses when it is no longer needed.
		///     </para>
		/// </remarks>
		...
		/// </param>
		/// <returns>
		/// Returns S_OK if the function succeeds, or an error value otherwise. Note that because this bitmap is not cached, if the window is not being previewed
		/// when an application calls this function, the function returns a success code but the bitmap is discarded and not used.
		/// </returns>
		/// <remarks>
		///     <para>
		/// A live preview (also known as a Peek preview) of a window appears when a user moves the mouse pointer over the window's thumbnail in the taskbar or
		/// gives the thumbnail focus in the ALT+TAB window. This view is a full-sized view of the window and can be a snapshot or an iconic representation.
		///     </para>
		///     <para>
		/// A window typically calls the DwmSetIconicLivePreviewBitmap function in response to a WM_DWMSENDICONICLIVEPREVIEWBITMAP message. The returned bitmap
		/// must not be larger than the client area of the window or frame and must have 32-bit color depth.
		///     </para>
		///     <para>
		/// The Desktop Window Manager (DWM) uses a copy of the bitmap, but the caller retains ownership of the original bitmap and is responsible for freeing
		/// the resources that it uses when it is no longer needed. The DWM does not keep its copy of the bitmap when the DWM stops displaying the live preview
		/// representation.
		///     </para>
		/// </remarks>
		...
		/// <summary>
		/// Called by an application to indicate that all previously provided iconic bitmaps from a window, both thumbnails and peek representations, should be
		/// refreshed.
		/// </summary>
		/// <param name="hwnd">
		...
		/// <remarks>
		///     <para>
		/// DwmFlush waits for any queued DirectX changes that were queued by the calling application to be drawn to the screen before returning. It does not
		/// flush the entire session rendering batch.
		///     </para>
		/// </remarks>
